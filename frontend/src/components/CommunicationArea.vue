...
const selectedEmail = ref(getUser().email)

watch(
  () => emailAccounts.data,
  (accounts) => {
    const def = (accounts || []).find(a => a.default_outgoing)
    selectedEmail.value = def?.email_id ?? getUser().email
  },
  { immediate: true }
)
