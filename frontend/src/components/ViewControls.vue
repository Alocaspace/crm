  ...
  >
   ...
   <Dialog
    ...
    :options="{
      ...
      actions: [
        {
          label: busy ? __('Processing…') : __('Import'),
          variant: 'solid',
          disabled: busy,
          onClick: () => importRows(),
        },
  ...
  >
    <template #body-content>
      ...
      <div class="mt-4 flex gap-2 items-center">
        <button class="bg-white rounded px-2 py-1" @click="downloadSampleCSV">
...
const csvFile = ref(null);  // To store the selected file

// Handle file selection and read it
...
async function importRows() {
  ...
  if (!csvFile.value) {
    importResult.value = 'Please select a CSV file first.';
    return;
  }
  ...
  try {
    ...
    const response = await fetch('http://crm.localhost:8080/api/method/crm.api.import.import_leads_from_csv_file', {
      method: 'POST', // Ensure POST method
      body: formData, // Use the FormData directly in the body
    });

    if (response.exc) throw new Error(response.exc);

    if(response.ok) showImportDialog(false)
  } catch (e) {
...
function downloadSampleCSV() {
  const sample =
`Salutation,First Name,Last Name,Email,Mobile No,Gender,Status,Organization,Website,Industry,Annual Rev,No. of Employees
Mr,John,Doe,john.doe@example.com,5551234567,Male,Open,Acme Corp,https://acme.example,Manufacturing,500000,200
`
