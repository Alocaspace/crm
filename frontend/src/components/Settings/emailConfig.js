...
export const services = [
  ...
  },
  {
    ...
    icon: LogoFrappeMail,
    info: __(
      `Setting up Webmail requires you to enable two factor authentication
		  and app specific passwords. Read more `,
    ),
    link: 'https://github.com/frappe/mail',
    custom: false,
...
export function validateInputs(state, isCustom) {
  ...
  if (!isCustom && state.service === "" && !state.imap_server) {
    return __('IMAP server is required')
  }
  if (!isCustom && state.service === "" && !state.imap_port) {
    return __('IMAP port is required')
  }
  if (!isCustom && state.service === "" && !state.smtp_server) {
    return __('SMTP server is required')
  }
  if (!isCustom && state.service === "" && !state.smtp_port) {
    return __('SMTP port is required')
  }
  if (isCustom) {
    if (!state.api_key) {
